...
void sha256_transform(UINT m[64], UINT H[8]) {
    #pragma unroll
    for (int i = 16; i < 64; i++) {
        m[i].i = SIG1(m[i - 2].i)
            + m[i - 7].i
...
void digest64(UINT data[64], UINT hash[8]) {
    hash[0].i = 0x6a09e667;
    hash[1].i = 0xbb67ae85;
    hash[2].i = 0x3c6ef372;
    hash[3].i = 0xa54ff53a;
...
void hash_to_hex(const UINT hash[8], UINT hex[64]) {    
    #pragma unroll
...
int down_branch(const uchar addr_char, uint* index, __global const uint *trie) {
    uint trie_data;
...
// the given term trie.
bool check_address(const HASH_CHAIN_T *chain,__global const uint *trie) {
